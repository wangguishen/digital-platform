<template>
  <el-submenu :index="menu.url">
    ...
    <template v-for="menu2 in menu.children">
      ...
      <el-menu-item v-else :key="menu2.id" :index="menu2.url" :route="{path: menu2.routerUrl}" @click="menuClick(menu2)">
        <template slot="title">
          <i :class="menu2.icon" />
...
export default {
  ...
  methods: {
    menuClick (menu2) {
      this.$store.dispatch('tagsView/setCurrentTitle', menu2.text)
    }
  }
}
</script>
